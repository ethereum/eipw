...
#[derive(Debug)]
pub struct RequiredIfNotEq<'b> {
    pub when: &'b str,
    pub notEquals: &'b str,
    pub then: &'b str,
}

impl<'n> Lint for RequiredIfNotEq<'n> {
    fn lint<'a, 'b>(&self, slug: &'a str, ctx: &Context<'a, 'b>) -> Result<(), Error> {
        ...
        match (when_opt, then_opt) {
            ...
            (Some(when), None) => {
                let label = format!(
                    "preamble header `{}` is required when `{}` is not `{}`",
                    self.then, self.when, self.notEquals,
                );
                ctx.report(Snippet {
            ...
            (Some(when), Some(then)) => {
                let label = format!(
                    "preamble header `{}` is only allowed when `{}` is not `{}`",
                    self.then, self.when, self.notEquals,
                );

                let info_label = format!("unless equal to `{}`", self.notEquals);
